...
sed -i "s/<username\/>/<username>root<\/username>/" /var/www/app/etc/config.xml
sed -i "s/<password\/>/<password>password<\/password>/g" /var/www/app/etc/config.xml

echo "Creating the magento database..."

echo "create database magento" | mysql -h db -P "$DB_PORT_3306_TCP_PORT"

while [ $? -ne 0 ]; do
	sleep 5
        echo "create database magento" | mysql -h db -P "$DB_PORT_3306_TCP_PORT"
        echo "show tables" | mysql -h db -P "$DB_PORT_3306_TCP_PORT" magento
done
